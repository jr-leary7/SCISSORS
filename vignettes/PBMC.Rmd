...
Now we pull the results back into R, and save them in our `Seurat` object.

```{r}
embed <- as.matrix(py$embed_multi)
rownames(embed) <- colnames(pbmc)
colnames(embed) <- c("Fit-SNE_1", "Fit-SNE_2")
pbmc@reductions$tsne_old <- pbmc@reductions$tsne
...
Here we'll examine clusters 0, 1, and 2. Cluster 0 seems large enough, and has enough variability on the X-axis of the t-SNE embedding to appear as though it might harbor subgroups. Clusters 1 and 2 have small but visible subclusters. We can see after running `ReclusterCells()` that we've separated out one subgroup from each of clusters 1 and 2, and three from cluster 0.
...
reclust_res <- ReclusterCells(pbmc, 
                              ...
                              redo.embedding = TRUE, 
                              random.seed = 629)
p2 <- DimPlot(reclust_res[[1]], cols = paletteer_d("nationalparkcolors::Badlands")) + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab() + 
      theme(legend.position = "bottom", 
            legend.justification = "center", 
            legend.direction = "horizontal") + 
      guides(color = guide_legend(nrow = 1, override.aes = list(size = 2)))
p3 <- DimPlot(reclust_res[[2]], cols = paletteer_d("nationalparkcolors::Badlands")) + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab() + 
      theme(legend.position = "bottom", 
            legend.justification = "center", 
            legend.direction = "horizontal") + 
      guides(color = guide_legend(nrow = 1, override.aes = list(size = 2)))
p4 <- DimPlot(reclust_res[[3]], cols = paletteer_d("nationalparkcolors::Badlands")) + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab() + 
      theme(legend.position = "bottom", 
            legend.justification = "center", 
...
Next, we'll reintegrate our new clusters into our original `Seurat` object - this requires some finagling as `Seurat` is a bit weird with how it stores cell-level metadata. Since we had six clusters originally, and we discovered five new subclusters, we'll end up with twelve total clusters.
...
Cluster 7 is only subtly separated from . We'll use differential expression testing to determine if the cells in cluster 7 are a spurious cluster, or a real CD4+ T cell subtype. After running a Wilcox test we can see that several of the differentially expressed are associated with the interferon family of virus-related signaling proteins, and thus with anti-viral immune activity.

```{r}
clust7_markers <- FindMarkers(pbmc, 
                              ident.1 = 7, 
                              ident.2 = 0, 
                              only.pos = TRUE, 
                              logfc.threshold = .75, 
...
```{r}
final_pal <- c("paleturquoise4", "goldenrod1", "steelblue1", "lightseagreen", 
               "sienna4", "mediumblue", "coral2", "magenta", "limegreen", 
               "forestgreen", "darkorchid3", "orange2")
...
saveSCISSORS <- function(plot = NULL, 
  ...
  if (pub.ready) {
    dir <- "~/Desktop/R/SCISSORS/vignettes/figures_pub/"
    if (!border) {
      plot <- plot + 
    ...
    dir <- "~/Desktop/R/SCISSORS/vignettes/figures_supp/"
    ggsave(filename = paste0(name, ".pdf"), 
           device = "pdf", 
           units = "in",
           path = dir, 
