...
We'll run SCISSORS on the monocytes & T cells; i.e. on clusters 1 & 5 and 0, 2, & 3. 

```{r, warning=FALSE, message=FALSE, results='hold'}
mono_reclust <- ReclusterCells(pbmc, 
                               which.clust = list(1, 5), 
                               merge.clusters = TRUE, 
                               n.HVG = 4000,
                               n.PC = 10, 
                               k.vals = c(10, 20, 30), 
...
# run Fit-SNE w/ perplexity annealing
init_mono = initialization.pca(pc_mono, random_state=629)
affin_anneal_mono = PerplexityBasedNN(pc_mono, perplexity=40, metric='cosine', random_state=629)
...
embed2_mono = embed1_mono.optimize(n_iter=750, exaggeration=1, momentum=0.8)
affin_anneal_mono.set_perplexity(20)
embed3_mono = embed2_mono.optimize(n_iter=100, exaggeration=3, momentum=0.6)
embed4_mono = embed3_mono.optimize(n_iter=250)

# run Fit-SNE w/ multiscale kernel - monocytes
# affin_multi_mono = Multiscale(pc_mono, perplexities=[20, 50], metric='cosine', random_state=629)
# init_mono = initialization.pca(pc_mono, random_state=629)
# tsne_obj_mono = TSNEEmbedding(init_mono, affin_multi_mono, negative_gradient_method='fft')
# embed_mono = tsne_obj_mono.optimize(n_iter=250, exaggeration=12, momentum=0.6)
# embed_mono2 = embed_mono.optimize(n_iter=50, exaggeration = 2)
# embed_multi_mono = embed_mono2.optimize(n_iter=750, exaggeration=1, momentum=0.8)

# run Fit-SNE w/ multiscale kernel - T cells
# affin_multi_t = Multiscale(pc_t, perplexities=[20, 50], metric='cosine', random_state=629)
# init_t = initialization.pca(pc_t, random_state=629)
# tsne_obj_t = TSNEEmbedding(init_t, affin_multi_t, negative_gradient_method='fft')
# embed_t = tsne_obj_t.optimize(n_iter=250, exaggeration=12, momentum=0.6)
# embed_t2 = embed_t.optimize(n_iter=50, exaggeration = 2)
# embed_multi_t = embed_t2.optimize(n_iter=750, exaggeration=1, momentum=0.8)

# run Fit-SNE w/ perplexity annealing
init_t = initialization.pca(pc_t, random_state=629)
affin_anneal_t = PerplexityBasedNN(pc_t, perplexity=40, metric='cosine', random_state=629)
...
embed2_t = embed1_t.optimize(n_iter=750, exaggeration=1, momentum=0.8)
affin_anneal_t.set_perplexity(20)
embed3_t = embed2_t.optimize(n_iter=100, exaggeration=3, momentum=0.6)
embed4_t = embed3_t.optimize(n_iter=250)
```

Now we bring the results back in to R.

```{r}
embed_mono <- as.matrix(py$embed4_mono)
