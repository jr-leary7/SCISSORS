...
Clusters 0, 1, and 2 have statistically significantly lower silhouette scores than the other three clusters. We also see a small clump of cells in Cluster 4 that has lower scores than the rest of the cluster. We'll focus on these four clusters when reclustering, as clusters 3 and 5 seem correct. 
...
```{r}
p0b <- DimPlot(pbmc, pt.size = 1.75, cols = paletteer_d("ggthemes::Classic_10")) + 
...
```{r}
p3 <- DimPlot(mono_reclust, pt.size = 1.75, cols = paletteer_d("ggsci::nrc_npg")) + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab() + 
      guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
p4 <- DimPlot(t_reclust, pt.size = 1.75, cols = paletteer_d("ggsci::nrc_npg")) + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab() + 
      guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
p3
...
```{r}
SaveFigure(p0a, name = "Seurat_Clusters_Silhouettes")
SaveFigure(p0b, name = "Seurat_Clusters")
SaveFigure(p1, name = "Seurat_Clusters_FitSNE")
SaveFigure(p2, name = "Marker_Genes_For_Seurat_Clusters")
...
SaveFigure(p24, name = "Marker_Genes_For_SCISSORS_Celltypes")
```
