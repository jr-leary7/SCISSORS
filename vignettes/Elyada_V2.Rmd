...
```{r, fig.width=8, fig.height=4}
pdac_markers_specific <- FindSpecificMarkers(pdac, de.method = "wilcox", log2fc.cutoff = 0.1)
...
top_macro_genes <- bed@assays@data@listData[["logcounts"]] %>% 
                   ...
                   mutate(gene = rownames(.)) %>% 
                   pull(gene)
caf_markers_final %<>% filter(!(gene %in% high_exp_genes), !(gene %in% top_macro_genes))
```
...
high_exp_genes <- unique(high_exp_genes)
pdac_ductal_markers_final %<>% filter(!(gene %in% high_exp_genes))
```
...
SaveFigure(my.plot = p25, name = "SCISSORS_Clusters_Stroma_Labels_FitSNE")
...
SaveFigure(my.plot = p45, name = "SCISSORS_Clusters_Ductal_Labels_FitSNE")
...
SaveFigure(my.plot = p47, name = "SCISSORS_Clusters_Ductal_Violin_Combined")
