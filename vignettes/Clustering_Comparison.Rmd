...
p0a <- DimPlot(pbmc_Seurat, reduction = "fitsne", group.by = "seurat_clusters", 
               pt.size = 0.75, cols = paletteer_d("ggsci::category20_d3")) + 
...
```{r}
p0b <- DimPlot(pbmc_SCISSORS, pt.size = 0.75, group.by = "seurat_clusters", 
...
sils_safe <- silhouette(dist = cos_dist, x = clust_ensembl$optimal_clustering)
```

Here's what the results from `SAFE` look like on our t-SNE embedding:

```{r}
p0c <- DimPlot(pbmc, pt.size = 0.75, group.by = "SAFE_clust", 
...
```{r}
p0d <- DimPlot(pbmc, pt.size = 0.75, group.by = "gini_clust", 
               reduction = "fitsne", cols = paletteer_d("ggsci::category20_d3")) + 
       labs(x = NULL, y = NULL, title = NULL) + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
