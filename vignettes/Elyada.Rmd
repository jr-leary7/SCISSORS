...
p7 <- DimPlot(pdac, reduction = "tsne", group.by = "malig") + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      scale_color_manual(values = c("firebrick3", "lightsteelblue")) + 
      theme(plot.title = element_blank()) + 
      theme_yehlab()
p7
```
...
Next, we use [Dr. Xianlu Peng's DECODER](https://github.com/laurapeng/decoderr) in order to deconvolve the dataset using NMF and assign weights to each cell. We calculate basal & classical PDAC, normal & activated stroma, immune, and endocrine & exocrine pancreas compartment weights. 
