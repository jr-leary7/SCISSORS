...
p0a <- ggplot(sil_df, aes(x = Cluster, y = Score, color = Cluster)) + 
       ...
       geom_signif(comparisons = list(c("1", "4"), c("1", "2"), c("0", "1"), c("3", "4"), c("4", "5")), 
                   test = wilcox.test, map_signif_level = TRUE, step_increase = .075) + 
       scale_y_continuous(limits = c(min(sil_df$Score), 1.5)) + 
...
```{r}
p0b <- DimPlot(pdac, reduction = "tsne", pt.size = 1.75) + 
