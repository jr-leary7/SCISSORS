...
p0a <- ggplot(sil_df, aes(x = Cluster, y = Score, color = Cluster)) + 
       geom_quasirandom() + 
       stat_summary(fun = "mean", geom = "crossbar", width = 0.5, color = "black") + 
       scale_color_paletteer_d("ggthemes::Classic_20") + 
       labs(y = "Silhouette Score", color = "Louvain Clusters", 
            caption = TeX(paste0("$\\bar{S}$ = ", round(mean(sil_df$Score), 2)))) + 
       theme_minimal() + 
...
Looking at the silhouette score distribution for each cluster, we see that Cluster 4 seems to have the best fit, and Clusters 2, 5, & 11 seem to have pretty poor fits. 

```{r}
p0a
```

Let's check out the t-SNE embedding. It looks decent, but could definitely be improved. Globally, the clusters are arranged in a blob - which isn't very informative - though the local structure seems to have been preserved fairly well. Clusters 5 and 11 are split in two, which mirrors what we saw in the previous plot, i.e. that those two clusters are not well fit. 

```{r}
p0b <- DimPlot(pdac, reduction = "tsne", pt.size = 0.75) + 
       scale_color_paletteer_d("ggthemes::Classic_20") + 
...
pdac@reductions$fitsne_multi <- CreateDimReducObject(embeddings = embed_multi, 
                                                     ...
                                                     assay = "SCT", 
                                                     global = TRUE)
p3 <- DimPlot(pdac, reduction = "fitsne_multi", pt.size = 0.75) + 
      scale_color_paletteer_d("ggthemes::Classic_20") +  
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab()+
      theme(legend.text = element_text(size = 10)) + 
...
embed6 = tsne3.optimize(n_iter=250, exaggeration=12, momentum=0.6)
...
pdac@reductions$fitsne_kern<- CreateDimReducObject(embeddings = embed_kern, 
                                                   ...
                                                   assay = "SCT", 
                                                   global = TRUE)
p4 <- DimPlot(pdac, reduction = "fitsne_kern", pt.size = 0.75) +
      scale_color_paletteer_d("ggthemes::Classic_20") + 
      labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
      theme_yehlab() +
      theme(legend.text = element_text(size = 10)) + 
...
We can easily define cluster 0 as the myCAF population, and clusters 1 and 2 as the slightly smaller iCAF population. Cluster 3 shows no enrichment whatsoever for either the iCAF or myCAF gene sets. 
...
So we have a small cluster of 14 cells that does not appear to express any of the fibroblast, CAF, perivascular, or endothelial markers. After performing differential expression analysis, we find that the top 3 markers for cluster 3 are CLU, CD74, and CRYAB. Interestingly, CLU and CD74 were found to be differentially expressed in the apCAF population discovered in the KPC mouse models of CAFs in Elyada *et al*.
...
The natural killers can be found in cluster 3 through their expression of PRF1 and NKG7. The NKG7 expression also confirms the identities of the CD8+ T cells we just annotated. 
...
nkt_norm$label <- case_when(nkt_norm$seurat_clusters == 0 ~ "Memory CD4+ T", 
                            ...
                            nkt_norm$seurat_clusters == 3 ~ "NK",
                            nkt_norm$seurat_clusters == 4 ~ "Naive CD4+ T",
...
embed_dc1 = tsne_dc.optimize(n_iter=250, exaggeration=10, momentum=0.6)
...
We can identify the classical monocytes in cluster 2 through their expression of CD14 and lack of expression of CD16 aka FCGR3A, expression of which reveals the larger group of CD16+ monocytes in cluster 0. . 
...
```{r}
mono_tumor$label <- case_when(mono_tumor$seurat_clusters == 0 ~ "CD16+ Monocyte", 
...
Lastly, we use expression of two [canonical cDC2 marker genes / transcription factors](https://onlinelibrary.wiley.com/doi/pdf/10.1111/imm.12888) to identify clusters 0, 1, and 2 as cDC cells, which are split by sample ID. 
...
p105 <- FeaturePlot(dc_tumor, reduction = "tsne", features = "KLF4") + 
        ...
        NoLegend() + 
        theme(axis.title = element_blank())
p106 <- DimPlot(dc_tumor, reduction = "tsne", group.by = "sample") + 
        scale_color_manual(values = paletteer_d("miscpalettes::brightPastel")) + 
        labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "Sample") + 
        theme_yehlab() + 
        NoLegend() + 
...
```{r}
dc_tumor$label <- case_when(dc_tumor$seurat_clusters == 0 ~ "cDC2", 
                            dc_tumor$seurat_clusters == 1 ~ "cDC2", 
                            dc_tumor$seurat_clusters == 2 ~ "cDC2", 
                            dc_tumor$seurat_clusters == 3 ~ "Langerhans-like DCB", 
                            dc_tumor$seurat_clusters == 4 ~ "Langerhans-like DCA", 
                            dc_tumor$seurat_clusters == 5 ~ "cDC1", 
...
Lastly, it seems we have a small group of CD8+ T cells in cluster 3 that snuck into the myeloid cluster, as defined by their expression of CD2, CD3D (marking them as NK / T cells), and CD8A & NKG7. I don't believe they're NK cells as they do not express PRF1 or GZMB. 
...
```{r}
saveSCISSORS(plot = p0, name = "Seurat_Clusters_tSNE", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p1, name = "Seurat_Clusters_UMAP", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p4, name = "Seurat_Clusters_FitSNE", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p5, name = "SingleR_Annos_sc_ref", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p6, name = "SingleR_Annos_bulk_ref", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p7, name = "CONICSmat_Annos", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p8, name = "DECODER_basal")
saveSCISSORS(plot = p9, name = "DECODER_classical")
saveSCISSORS(plot = p10, name = "DECODER_exocrine")
saveSCISSORS(plot = p11, name = "DECODER_endocrine")
saveSCISSORS(plot = p12, name = "DECODER_immune")
saveSCISSORS(plot = p13, name = "DECODER_normal_stroma")
saveSCISSORS(plot = p14, name = "DECODER_act_stroma")
saveSCISSORS(plot = p15, name = "Fibro_all_cells_COL1A1")
saveSCISSORS(plot = p16, name = "Fibro_all_cells_COL3A1")
saveSCISSORS(plot = p17, name = "Fibro_all_cells_LUM")
saveSCISSORS(plot = p18, name = "Fibro_all_cells_DCN")
saveSCISSORS(plot = p19, name = "Fibro_SCISSORS_FitSNE")
saveSCISSORS(plot = p20, name = "Fibro_iCAF_VAM")
saveSCISSORS(plot = p21, name = "Fibro_myCAF_VAM")
saveSCISSORS(plot = p22, name = "Fibro_apCAF_VAM")
saveSCISSORS(plot = p23, name = "Fibro_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p24, name = "Endo_Perivascular_SCISSORS_FitSNE")
saveSCISSORS(plot = p25, name = "Perivascular_IGFBP7")
saveSCISSORS(plot = p26, name = "Perivascular_ACTA2")
saveSCISSORS(plot = p27, name = "Perivascular_RGS5")
saveSCISSORS(plot = p28, name = "Endothelial_PLVAP")
saveSCISSORS(plot = p29, name = "Endothelial_VWF")
saveSCISSORS(plot = p30, name = "Endo_Perivascular_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p31, name = "NKT_all_cells_CD3D")
saveSCISSORS(plot = p32, name = "NKT_Tumor_SCISSORS_FitSNE")
saveSCISSORS(plot = p33, name = "NKT_Tumor_CD4T_IL7R")
saveSCISSORS(plot = p34, name = "NKT_Tumor_CD4T_CD69")
saveSCISSORS(plot = p35, name = "NKT_Tumor_Treg_IL2RA")
saveSCISSORS(plot = p36, name = "NKT_Tumor_Treg_FOXP3")
saveSCISSORS(plot = p37, name = "NKT_Tumor_Prolif_Treg_TOP2A")
saveSCISSORS(plot = p38, name = "NKT_Tumor_Mast_TPSAB1")
saveSCISSORS(plot = p39, name = "NKT_Tumor_NK_NKG7")
saveSCISSORS(plot = p40, name = "NKT_Tumor_NK_PRF1")
saveSCISSORS(plot = p41, name = "NKT_Tumor_CD8T_CD8A")
saveSCISSORS(plot = p42, name = "NKT_Tumor_CD8T_CD2")
saveSCISSORS(plot = p43, name = "NKT_Tumor_Intermediate_Mono_LYZ")
saveSCISSORS(plot = p44, name = "NKT_Tumor_Intermediate_Mono_HLADRA")
saveSCISSORS(plot = p45, name = "NKT_Tumor_Intermediate_Mono_CD74")
saveSCISSORS(plot = p46, name = "NKT_Tumor_Intermediate_Mono_HLADPB1")
saveSCISSORS(plot = p47, name = "NKT_Tumor_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p48, name = "NKT_Norm_SCISSORS_FitSNE")
saveSCISSORS(plot = p49, name = "NKT_Norm_CD4T_IL7R")
saveSCISSORS(plot = p50, name = "NKT_Norm_CD4T_S100A4")
saveSCISSORS(plot = p51, name = "NKT_Norm_CD4T_CCR7")
saveSCISSORS(plot = p52, name = "NKT_Norm_CD8T_CD8A")
saveSCISSORS(plot = p53, name = "NKT_Norm_Treg_TIGIT")
saveSCISSORS(plot = p54, name = "NKT_Norm_Treg_FOXP3")
saveSCISSORS(plot = p55, name = "NKT_Norm_NK_PRF1")
saveSCISSORS(plot = p56, name = "NKT_Norm_NK_NKG7")
saveSCISSORS(plot = p57, name = "NKT_Norm_Prolif_Treg_TOP2A")
saveSCISSORS(plot = p58, name = "NKT_Norm_Intermediate_Mono_LYZ")
saveSCISSORS(plot = p59, name = "NKT_Norm_Intermediate_Mono_HLADRA")
saveSCISSORS(plot = p60, name = "NKT_Norm_Intermediate_Mono_LST1")
saveSCISSORS(plot = p61, name = "NKT_Norm_Intermediate_Mono_TYROBP")
saveSCISSORS(plot = p62, name = "NKT_Norm_SCISSORS_FitSNE", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p63, name = "Ductal_all_cells_KRT8")
saveSCISSORS(plot = p64, name = "Ductal_SCISSORS_FitSNE")
saveSCISSORS(plot = p65, name = "Ductal_Lipid_Proc_ANPEP")
saveSCISSORS(plot = p66, name = "Ductal_Secretory_SOD3")
saveSCISSORS(plot = p67, name = "Ductal_Secretory_CFTR")
saveSCISSORS(plot = p68, name = "Ductal_Classical1_TFF1")
saveSCISSORS(plot = p69, name = "Ductal_Classical1_TFF2")
saveSCISSORS(plot = p70, name = "Ductal_Classical1_SampleID")
saveSCISSORS(plot = p71, name = "Ductal_Classical2_CRISP3")
saveSCISSORS(plot = p72, name = "Ductal_basal_DECODER")
saveSCISSORS(plot = p73, name = "Ductal_CD16_Mono_CD14")
saveSCISSORS(plot = p74, name = "Ductal_CD16_Mono_FCGR3A")
saveSCISSORS(plot = p75, name = "Ductal_CD16_Mono_MS4A7")
saveSCISSORS(plot = p76, name = "Ductal_Acinar_CTRB2")
saveSCISSORS(plot = p77, name = "Ductal_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p78, name = "Plasma_all_cells_IGJ")
saveSCISSORS(plot = p79, name = "Plasma_all_cells_IRF7")
saveSCISSORS(plot = p80, name = "Plasma_SCISSORS_FitSNE")
saveSCISSORS(plot = p81, name = "Plasma_Plasma_JCHAIN")
saveSCISSORS(plot = p82, name = "Plasma_pDC_IGJ")
saveSCISSORS(plot = p83, name = "Plasma_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p84, name = "B_all_cells_MS4A1")
saveSCISSORS(plot = p85, name = "B_all_cells_CD79A")
saveSCISSORS(plot = p86, name = "B_all_cells_tissue_type")
saveSCISSORS(plot = p87, name = "Myeloid_Tumor_original_FitSNE")
saveSCISSORS(plot = p88, name = "Myeloid_Tumor_Resident_Macro_CD14")
saveSCISSORS(plot = p89, name = "Myeloid_Tumor_Resident_Macro_C1QA")
saveSCISSORS(plot = p90, name = "Myeloid_Tumor_Classic_Mono_LYZ")
saveSCISSORS(plot = p91, name = "Myeloid_Tumor_Classic_Mono_S100A8")
saveSCISSORS(plot = p92, name = "Myeloid_Tumor_Alternative_Macro_Spp1")
saveSCISSORS(plot = p93, name = "Myeloid_Tumor_DC_FCER1A")
saveSCISSORS(plot = p94, name = "Monocyte_Tumor_SCISSORS_FitSNE")
saveSCISSORS(plot = p95, name = "DC_Tumor_SCISSORS_FitSNE")
saveSCISSORS(plot = p96, name = "Monocyte_Tumor_Neutrophil_S100A8")
saveSCISSORS(plot = p97, name = "Monocyte_Tumor_Neutrophil_S100A9")
saveSCISSORS(plot = p98, name = "Monocyte_Tumor_Classic_Mono_CD14")
saveSCISSORS(plot = p99, name = "Monocyte_Tumor_CD16_Mono_FCGR3A")
saveSCISSORS(plot = p100, name = "DC_Tumor_cDC1_CLEC9A")
saveSCISSORS(plot = p101, name = "DC_Tumor_Langerhans_DC_CD1A")
saveSCISSORS(plot = p102, name = "DC_Tumor_Langerhans_DC_CD207")
saveSCISSORS(plot = p103, name = "DC_Tumor_Activated_DC_LAMP3")
saveSCISSORS(plot = p104, name = "DC_Tumor_cDC2_CLEC10A")
saveSCISSORS(plot = p105, name = "DC_Tumor_cDC2_KLF4")
saveSCISSORS(plot = p106, name = "DC_Tumor_cDC2_SAMPLEID")
saveSCISSORS(plot = p107, name = "Mono_Tumor_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p108, name = "DC_Tumor_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p109, name = "Mono_Norm_SCISSORS_FitSNE")
saveSCISSORS(plot = p110, name = "DC_Norm_SCISSORS_FitSNE")
saveSCISSORS(plot = p111, name = "Mono_Norm_Neutrophil_S100A8")
saveSCISSORS(plot = p112, name = "Mono_Norm_Neutrophil_S100A9")
saveSCISSORS(plot = p113, name = "Mono_Norm_Classic_Monocyte_CD68")
saveSCISSORS(plot = p114, name = "Mono_Norm_Classic_Monocyte_CD14")
saveSCISSORS(plot = p115, name = "Mono_Norm_Classic_MOnocyte_SampleID")
saveSCISSORS(plot = p116, name = "Mono_Norm_CD8T_CD2")
saveSCISSORS(plot = p117, name = "Mono_Norm_CD8T_CD3D")
saveSCISSORS(plot = p118, name = "Mono_Norm_CD8T_CD8A")
saveSCISSORS(plot = p119, name = "Mono_Norm_CD8T_NKG7")
saveSCISSORS(plot = p120, name = "DC_Norm_cDC1_CLEC9A")
saveSCISSORS(plot = p121, name = "DC_Norm_cDC2_CLEC10A")
saveSCISSORS(plot = p122, name = "Mono_Norm_final_labels", pub.ready = TRUE, border = FALSE)
saveSCISSORS(plot = p123, name = "DC_Norm_final_labels", pub.ready = TRUE, border = FALSE)
```
