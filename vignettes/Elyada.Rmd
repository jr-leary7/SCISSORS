...
We run the typical single cell pre-processing steps on our cells - normalization, dimension reduction, and clustering. The t-SNE embedding looks OK, but could definitely be improved. Globally, the clusters are arranged in a blob - which isn't very informative - though the local structure seems to have been preserved fairly well. Cluster 4 is split in two, which we would prefer not to have happen. 
...
It seems like UMAP does a good job of clearly separating our clusters and preserving the global structure of the data. However, clusters 4 and 7 are still separated, and it's difficult to see local structure within some of the clusters due to their density.
...
The embedding looks good, but the subclusters within cluster 4 are still clearly separated, and a small part of cluster 7 is far from the rest of the cells in the cluster.
...
```{python}
affin_anneal = PerplexityBasedNN(pc_df, perplexity=80, metric='cosine', random_state=629)
...
The perplexity annealing hasn't really improved the embedding much - cluster 4 is still scattered over the two-dimensional space, and the outlier subgroup in cluster 7 hasn't moved any closer to the rest of the cluster. 
...
The multiscale trick doesn't really fix the embedding issues either, but it does clean up a bit of the noise in the plot, so even though clusters 4 & 7 are still split up, we'll use this embedding going forward and make sure to examine those clusters for subgroups using `SCISSORS`.
...
Here we use the `SingleR` package to identify broad cell types. The reference dataset we load is an `sctransform` normalized version of the raw counts available in `scRNAseq::BaronPancreasData()` from @baron2016, which consists of normal pancreas cells that were sequenced and annotated by the researchers.
...
This dataset is composed of labeled bulk RNA-seq samples from the Human Primary Cell Atlas.
...
The bulk reference gives us a bit more granular labels for the immune cells, and confirms the identities of the ductal / epithelial and fibroblast (denoted mesenchymal stem cells here) clusters. 
...
bic_table <- read.table("./Elyada_CNVs_BIC_LR.txt", 
                        ...
                        header = TRUE, 
                        check.names = FALSE)
cand_regions <- rownames(bic_table[bic_table$`BIC difference` > 300 & 
                                     bic_table$`LRT adj. p-val` < .01, ])
...
```{r, echo=FALSE}
rm(cpm, bic_table, chrom_regions, cnv_est, gene_pos, 
   cand_regions, hist1, normal, norm_factor, malignant)
```
...
Going back to the main `Seurat` object, we have a large population of T and NK cells that warrants further investigation. Using NKG7 expression we can easily identify cluster 0 as the natural killer population, so we hypothesize that clusters 3, 4, and 9 contain our T cells. We already see some good separation, so reclustering the cells should have good results. We'll also split our NK & T population by the tissue condition - tumor vs. adjacent normal - as the original authors did.

### Tumor

```{r}
nkt_tumor <- subset(pdac, subset = seurat_clusters %in% c(0, 3, 4, 9) & 
                                   condition == "PDAC")
```
...
embed_nkt_tumor1 = tsne_nkt_tumor.optimize(n_iter=350, exaggeration=12, momentum=0.5) 
embed_nkt_tumor2 = embed_nkt_tumor1.optimize(n_iter=950, momentum=0.8)
affin_nkt_tumor.set_perplexity(30)
embed_nkt_tumor3 = embed_nkt_tumor2.optimize(n_iter=500)
```

The reembedding isn't perfect, which we somewhat expect as immune cells are difficult to tell apart based on the transcriptome alone. 
...
CLusters 0 and 2 contain our CD4+ T cells, which we characterize using IL7R as we did in the PBMC3k vignette. It's outside of our scope here to determine which subtype each cluster belongs to, so we'll simply assign both clusters the same label and move on. 
...
Mast cells can be identified using CPA3 expression in cluster 6. 

```{r}
p37 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "CPA3") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
NKG7 and PRF1 show us the NK cells in cluster 4. 
...
We use CD8A and CD2 to reveal the CD8+ T cells. 
...
Lastly, it appears that some myeloid cells were grouped with the larger NK/T cluster in the original clustering. We determined the small myeloid cluster first through expression of LYZ, and then showed it to be composed of resident macrophages using C1QA and APOE. 

```{r}
p42 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "LYZ") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p43 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "C1QA") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p44 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "APOE") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
nkt_tumor$label <- case_when(nkt_tumor$seurat_clusters == 0 ~ "CD4+ T", 
                             nkt_tumor$seurat_clusters == 1 ~ "CD8+ T", 
                             nkt_tumor$seurat_clusters == 2 ~ "CD4+ T", 
                             nkt_tumor$seurat_clusters == 3 ~ "T-reg", 
                             nkt_tumor$seurat_clusters == 4 ~ "NK", 
                             nkt_tumor$seurat_clusters == 5 ~ "Resident Macrophage", 
                             nkt_tumor$seurat_clusters == 6 ~ "Mast", 
                             nkt_tumor$seurat_clusters == 7 ~ "Proliferating T-reg")
p45 <- DimPlot(nkt_tumor, reduction = "tsne", group.by = "label") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
```{r}
nkt_norm <- subset(pdac, subset = seurat_clusters %in% c(0, 3, 4, 9) & condition == "AdjNorm")
```
...
# run Fit-SNE
affin_nkt_norm = PerplexityBasedNN(nkt_norm_pc, perplexity=300, metric='cosine', random_state=629)
...
affin_nkt_norm.set_perplexity(15)
embed_nkt_norm3 = embed_nkt_norm2.optimize(n_iter=1200, exaggeration=1, momentum=0.8)
```
...
nkt_norm@reductions$tsne<- CreateDimReducObject(embeddings = embed_nkt_norm, 
                                                ...
                                                assay = "SCT", 
                                                global = TRUE)
p46 <- DimPlot(nkt_norm, reduction = "tsne") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 3)))
...
We can use IL7R expression to identify the CD4+ T cells in clusters 0, 1, and 7. 

```{r}
p47 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "IL7R") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
We can further delineate clusters 0 and 1 into memory CD4+ T cells using S100A4. Notice that cluster 7 does not express S100A4 at all. 

```{r}
p48 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "S100A4") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
Finally, we can define cluster 7 as comprising of naive CD4+ T cells through its expression of CCR7. 

```{r}
p49 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CCR7") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
Next we reveal the CD8+ T cells in cluster 2 with CD8A, as per usual. 

```{r}
p50 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CD8A") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
The T-reg cluster, cluster 5, is identified using IL2RA and FOXP3. 

```{r}
p51 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "IL2RA") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p52 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "FOXP3") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
The natural killers can be found in cluster 8 through their expression of PRF1.

```{r}
p53 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "PRF1") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
The tiny proliferating T-reg population in cluster 8 is characterized by TOP2A. 

```{r}
p54 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "TOP2A") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
Like with the tumor NK/T cells, some myeloid cell types were mistakenly grouped in with our NK/T cells in the original clustering. First we identify the resident macrophages in cluster 3 first using expression of LYZ (a myeloid marker) and then specifically through APOE and C1QA expression. 

```{r}
p55 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "LYZ") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
```{r}
p56 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "APOE") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p57 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "C1QA") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
Lastly, we define cluster 4 as containing marginal zone B cells. We identified the cluster as being composed of B cells first through its expression of CD79A and MS4A1, and then specifically as marginal zone B cells using its joint expression of CD19, CD21 (aka CR2), and CD24. 

```{r}
p58 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CD79A") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p59 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "MS4A1") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
```{r}
p60 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CD19") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p61 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CR2") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p62 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CD24") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
nkt_norm$label <- case_when(nkt_norm$seurat_clusters == 0 ~ "Memory CD4+ T", 
                            nkt_norm$seurat_clusters == 1 ~ "Memory CD4+ T",
                            nkt_norm$seurat_clusters == 2 ~ "CD8+ T",
                            nkt_norm$seurat_clusters == 3 ~ "Resident macrophage",
                            nkt_norm$seurat_clusters == 4 ~ "Marginal zone B",
                            nkt_norm$seurat_clusters == 5 ~ "T-reg",
                            nkt_norm$seurat_clusters == 6 ~ "NK",
                            nkt_norm$seurat_clusters == 7 ~ "Naive CD4+ T",
                            nkt_norm$seurat_clusters == 8 ~ "Proliferating T-reg")
p63 <- DimPlot(nkt_norm, reduction = "tsne", group.by = "label") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 2, override.aes = list(size = 3)))
...
```{r}
ductal_reclust <- ReclusterCells(pdac, 
...
```{r}
duct_pc <- Embeddings(ductal_reclust, reduction = "pca")
```
...
duct_pc = r.duct_pc
# run Fit-SNE
affin_duct = PerplexityBasedNN(duct_pc, perplexity=30, metric='cosine', random_state=629)
init = initialization.pca(duct_pc, random_state=629)
tsne_duct = TSNEEmbedding(init, affin_duct, negative_gradient_method='fft')
embed_d1 = tsne_duct.optimize(n_iter=450, exaggeration=12, momentum=0.7)
embed_d2 = embed_d1.optimize(n_iter=500, exaggeration=2, momentum=0.8)
affin_duct.set_perplexity(60)
embed_d3 = embed_d2.optimize(n_iter=500, momentum=0.8)
```
...
rownames(embed_duct) <- colnames(ductal_reclust)
ductal_reclust@reductions$bh_tsne <- ductal_reclust@reductions$tsne
ductal_reclust@reductions$tsne<- CreateDimReducObject(embeddings = embed_duct, 
                                                      key = "FitSNE_", 
                                                      assay = "SCT", 
                                                      global = TRUE)
p64 <- DimPlot(ductal_reclust, reduction = "tsne") + 
...
```{r}
ductal_markers <- FindAllMarkers(ductal_reclust, 
                                 assay = "SCT", 
                                 logfc.threshold = .75, 
                                 test.use = "wilcox", 
                                 only.pos = TRUE, 
...
```{r}
p65 <- FeaturePlot(ductal_reclust, reduction = "tsne", features = "ANPEP") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
```{r}
p66 <- FeaturePlot(ductal_reclust, reduction = "tsne", features = "SOD3") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
We use the classical PDAC compartment weights from `DECODER` to identify the classical cells in clusters 0, 1, and 5. One of these clusters will be further identified as the classical 2 subtype found by the authors. It is out of our scope to determine what's driving the difference in gene expression between the two classical 1 clusters.

```{r}
p67 <- FeaturePlot(ductal_reclust, reduction = "tsne", features = "classical") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
...
The classical 2 cells are located in cluster 5, as evidenced by their expression of CRISP3. 

```{r}
p68 <- FeaturePlot(ductal_reclust, reduction = "tsne", features = "CRISP3") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend() + 
...
```{r}
p69 <- FeaturePlot(ductal_reclust, reduction = "tsne", features = "basal") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
...
```{r}
ductal_reclust$label <- case_when(ductal_reclust$seurat_clusters == 0 ~ "Classical 1", 
                                  ductal_reclust$seurat_clusters == 1 ~ "Classical 1", 
                                  ductal_reclust$seurat_clusters == 2 ~ "Secretory", 
                                  ductal_reclust$seurat_clusters == 3 ~ "Basal-like", 
                                  ductal_reclust$seurat_clusters == 4 ~ "Lipid Proc.", 
                                  ductal_reclust$seurat_clusters == 5 ~ "Classical 2")
p70 <- DimPlot(ductal_reclust, reduction = "tsne", group.by = "label") + 
       scale_color_manual(values = paletteer_d("ggsci::nrc_npg")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
```{r}
p71 <- FeaturePlot(pdac, reduction = "tsne", features = "JCHAIN") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "JCHAIN") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p72 <- FeaturePlot(pdac, reduction = "tsne", features = "IRF7") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
...
plas_pc = r.plas_pc
# run Fit-SNE
affin_plas = PerplexityBasedNN(plas_pc, perplexity=50, metric='cosine', random_state=629)
...
embed_p2 = embed_p1.optimize(n_iter=950, exaggeration=1, momentum=0.8)
```
...
plas@reductions$tsne<- CreateDimReducObject(embeddings = embed_plas, 
                                            ...
                                            assay = "SCT", 
                                            global = TRUE)
p73 <- DimPlot(plas, reduction = "tsne") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 3)))
...
```{r}
p74 <- FeaturePlot(plas, reduction = "tsne", features = "JCHAIN") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "JCHAIN") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p75 <- FeaturePlot(plas, reduction = "tsne", features = "IRF7") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "IRF7") + 
       theme_yehlab() + 
       NoLegend() + 
...
p76 <- DimPlot(plas, reduction = "tsne", group.by = "label") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 3)))
...
We mentioned at the very beginning of our analysis that the cells in cluster 5 looked like they could be easily subtyped. We'll run `SCISSORS`, then examine expression patterns and see if our hypothesis is correct. But first, we'll broadly identify the cluster as B cells using MS4A1 expression. 

```{r}
p77 <- FeaturePlot(pdac, reduction = "tsne", features = "MS4A1") +
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "IRF7") + 
       theme_yehlab() + 
       NoLegend() + 
...
First we run `SCISSORS`. 

```{r}
b <- ReclusterCells(pdac, 
                    which.clust = 5, 
                    n.PC = 20, 
                    which.dim.reduc = "tsne", 
                    redo.embedding = TRUE, 
                    do.plot = FALSE, 
                    random.seed = 629)
b_pc <- Embeddings(b, reduction = "pca")
```

As with the other clusters, we run Fit-SNE.

```{python}
# import data
b_pc = r.b_pc
# run Fit-SNE
affin_b = PerplexityBasedNN(b_pc, perplexity=100, metric='cosine', random_state=629)
init = initialization.pca(b_pc, random_state=629)
tsne_b = TSNEEmbedding(init, affin_b, negative_gradient_method='fft')
embed_b1 = tsne_b.optimize(n_iter=250, exaggeration=12, momentum=0.6)
embed_b2 = embed_b1.optimize(n_iter=950, exaggeration=1, momentum=0.8)
affin_b.set_perplexity(30)
embed_b3 = embed_b2.optimize(n_iter=500)
```

We pull the Fit-SNE results back into R and plot our clusters. 

```{r}
embed_b <- as.matrix(py$embed_b3)
rownames(embed_b) <- colnames(b)
b@reductions$bh_tsne <- b@reductions$tsne
b@reductions$tsne<- CreateDimReducObject(embeddings = embed_b, 
                                         key = "FitSNE_", 
                                         assay = "SCT", 
                                         global = TRUE)
p78 <- DimPlot(b, reduction = "tsne") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 3)))
...
Here we specifically look for CD-family genes that distinguish our clusters. Specifically, we see that cluster 0 is CD24+ and cluster 1 is CD69+. 

```{r}
b_markers <- FindAllMarkers(b, 
                            assay = "SCT", 
                            logfc.threshold = .25, 
                            only.pos = TRUE, 
                            verbose = FALSE, 
                            random.seed = 629) %>% 
             subset(p_val_adj < .05 & grepl("^CD", gene)) %>% 
             print()
```

Both clusters are CD19+. 

```{r}
p79 <- FeaturePlot(b, reduction = "tsne", features = "CD19") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "CD19") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p79 / p78
...
CD69 and CD24 expression allow us to identify the CD19+ CD69+ activated B cells along with the CD19+ CD24+ B-regulatory cells. 

```{r}
p80 <- FeaturePlot(b, reduction = "tsne", features = "CD69") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "CD69") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p81 <- FeaturePlot(b, reduction = "tsne", features = "CD24") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "CD24") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
(p80 | p81) / p78
...
We add final labels to our B cell `Seurat` object, then visualize the results. 

```{r}
b$label <- case_when(b$seurat_clusters == 0 ~ "B-reg", 
                     b$seurat_clusters == 1 ~ "Activated B")
p82 <- DimPlot(b, reduction = "tsne", group.by = "label") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
First we'll attempt to assign broad cell type labels to each of the three putative myeloid clusters. We'll use the marker genes from Elyada *et al* once again. 

```{r}
myo_tumor <- subset(pdac, subset = seurat_clusters %in% c(1, 2, 8) & condition == "PDAC")
p83 <- DimPlot(myo_tumor, reduction = "tsne") + 
       scale_color_manual(values = paletteer_d("awtools::mpalette")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
```{r}
p84 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "CD14") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "CD14") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p85 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "C1QA") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
...
```{r}
p86 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "LYZ") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2", title = "LYZ") + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p87 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "S100A8") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
...
```{r}
p88 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "FCER1A") + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
...
myo_tumor_reclust <- ReclusterCells(myo_tumor, 
                                    ...
                                    n.PC = 20, 
                                    k.vals = c(5, 10, 20), 
                                    resolution.vals = c(.2, .25, .3), 
                                    n.variable.genes = 4000, 
                                    which.dim.reduc = "tsne", 
                                    redo.embedding = TRUE, 
                                    do.plot = FALSE, 
                                    random.seed = 629)
DimPlot(myo_tumor_reclust[[1]], reduction = "tsne")
DimPlot(myo_tumor_reclust[[2]], reduction = "tsne")
DimPlot(myo_tumor_reclust[[3]], reduction = "tsne")
FeaturePlot(myo_tumor_reclust[[3]], reduction = "tsne", features = "IDO1")
macro_tumor <- myo_tumor_reclust[[1]]
mono_tumor <- myo_tumor_reclust[[2]]
dc_tumor <- myo_tumor_reclust[[3]]
```
...
saveSCISSORS(plot = p25, name = "Endothelial_PLVAP.pdf")
saveSCISSORS(plot = p27, name = "Endothelial_VWF.pdf")
saveSCISSORS(plot = p28, name = "Fibro_apCAF_VAM.pdf")
saveSCISSORS(plot = p29, name = "Fibro_SCISSORS_annos.pdf")
```
